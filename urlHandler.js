...
let maxShortUrl = 0; // the current largest short_url value 
...
const shortUrlUpdater = function() {
  UrlEntry
    ...
    .exec(function(err, data) {
      ...
      if (data.length > 0) {
        // if there is some data in the database already 
        // save the max value to the variable:
        maxShortUrl = data[0].short_url;
      }
      else {
        // if we receive an empty array back,leave maxShortUrl = 0 
      }
  });  
};
